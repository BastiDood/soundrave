  ...
  if (error instanceof NavigationError)
    ...
    switch (error.type) {
      ...
      case API_ERROR_TYPES.INIT_FAILED:
        description = new SafeString('We have encountered an error while initializing your session. We did not expect this behavior, so we have logged you out just to be safe. You may <a href=/login>log back in</a>, but if this issue still persists, please contact the webmaster.');
        break;
      case API_ERROR_TYPES.REFRESH_FAILED:
        description = new SafeString('We have encountered an error while refreshing your session. We did not expect this behavior, so we have logged you out just to be safe. You may <a href=/login>log back in</a>, but if this issue still persists, please contact the webmaster.');
        break;
      case API_ERROR_TYPES.NO_PERMISSION:
        description = new SafeString('We do not have the permission to access some information about you. You probably tampered with the redirect URL at Spotify. Although we must commend you for your sneaky attempt to fool our servers, we must invalidate your session just to be safe. You may <a href=/login>log back in</a>, but this time, please be careful. However, if you did not expect to see this issue, please contact the webmaster.');
        break;
      case API_ERROR_TYPES.UNAUTHORIZED:
        description = new SafeString('We do not have the permission to access some information about you. You probably tampered with the redirect URL at Spotify. Although we must commend you for your sneaky attempt to fool our servers, we must invalidate your session just to be safe. You may <a href=/login>log back in</a>, but this time, please be careful. However, if you did not expect to see this issue, please contact the webmaster.');
        break;
      case API_ERROR_TYPES.FORBIDDEN:
        description = new SafeString('Spotify has blocked our request to access some information about you. We did not expect this behavior, so we have logged you out just to be safe. You may <a href=/login>log back in</a>, but if this issue still persists, please contact the webmaster.');
        break;
      case API_ERROR_TYPES.NOT_FOUND:
        description = new SafeString('We cannot find some information about your favorite artists. We did not expect this behavior, so we have logged you out just to be safe. As an extra precaution, we have also removed everything we know about you and your profile from our databases. You may <a href=/login>log back in</a>, but if this issue still persists, please contact the webmaster.');
        break;
