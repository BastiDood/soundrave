const { findDistinctElementsFromArrays } = require('../../src/util/findDistinctElementsFromArrays');
...
describe('Two arrays', () => {
  test('Remove 3 of 5 strings from first array', () => {
    ...
    expect(findDistinctElementsFromArrays(arr1, arr2)).toEqual(expected);
  });
...
describe('Three arrays', () => {
  test('Find elements that have not been duplicated from 3 arrays of length 3', () => {
    ...
    const arr3 = [ '3', '4', '5' ];
    const expected = [ '1', '5' ];
    expect(findDistinctElementsFromArrays(arr1, arr2, arr3)).toEqual(expected);
  });
...
describe('Blank arrays', () => {
  test('One blank array', () => {
    expect(findDistinctElementsFromArrays([])).toEqual([]);
  });
  ...
  test('Blank arrays do not alter other arrays', () => {
    ...
    expect(findDistinctElementsFromArrays(arr1, [])).toEqual(arr1);
  });
});
